...
static inline float wcinput_event_normalized(wcinput_event_t event, float out_min, float out_max){
...
static inline int wcinput_ctx_init(wcinput_ctx_t* ctx){
    if (wcvec_init(&ctx->devices, sizeof(wcinput_device_t))) return -1;
    if (wcque_init(&ctx->events, sizeof(wcinput_event_t))) return -1;
    return 0;
}
static inline void wcinput_ctx_free(wcinput_ctx_t* ctx){
    wcvec_free(&ctx->devices);
    wcque_free(&ctx->events);
}

static inline int wcinput_ctx_find_devices(wcinput_ctx_t* ctx){
    struct dirent* entry;
    DIR* folder;
    int rc = 1;
    int fd;
...
static inline int wcinput_ctx_poll(wcinput_ctx_t* ctx, wcinput_event_t* out){
