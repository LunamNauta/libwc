...
int main(){
    ...
    while (true){
        ...
        right_accel = fabsf(right_accel) < 1e-1f ? 0.0f : right_accel;
        left_accel = fabsf(left_accel) < 1e-1f ? 0.0f : left_accel;
        right_duty += ((-right_accel) - right_duty) * 1e-5f;
        left_duty += ((-left_accel) - left_duty) * 1e-5f;
        if (right_accel == 0.0f) right_duty = fabsf(right_duty) < 1e-2f ? 0.0f : right_duty;
        if (left_accel == 0.0f) left_duty = fabsf(left_duty) < 1e-2f ? 0.0f : left_duty;
        ...
        while (!wcinput_ctx_pop_event(&ctx, &event)){
            ...
            if (event.ev.type == EV_KEY && event.ev.code == BTN_MODE) break;
