...
int wccan_read_bus(const wccan_t* ctx, wccan_frame_t* out_frame){
    return read(ctx->socket, &out_frame->frame, sizeof(struct can_frame)) == sizeof(struct can_frame) ? 0 : -1;
}
int wccan_write_bus(const wccan_t* ctx, const wccan_frame_t* frame){
    return write(ctx->socket, &frame->frame, sizeof(struct can_frame)) == sizeof(struct can_frame) ? 0 : -1;
}
